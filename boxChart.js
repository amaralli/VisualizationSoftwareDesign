window.boxChart = (function() {
	...
	var myChart = function() {
		...
		//filename as chosen by the user
		var fileName;
		...
		function makeChart(selection) {
		    ...
		    var scale = function() {
				...
				xScale = d3.scale.ordinal().rangeBands([0, width], .1).domain(xVals);
			...
			var draw = function() {
			    ...
			    rect.append('rect')
			    	.attr('x', function(d) {
			    		return margin.left + xScale(d[xVar]);
			    	})
			    	.attr('y', 0)
			    	.attr('height', 0)
		    	...
		    	rect.transition()
		    		...
		    		.attr('width', xScale.rangeBand())
		    		.attr('title', function(d) {return d[xVar]})
		    		.style('fill', color);
			    ...
			    minMaxMarkers.append('circle')
				    	.attr('cx', function(d) {
				    		return margin.left + xScale(d[xVar]) + (0.5 * xScale.rangeBand());
				    	})
				...
				medianLine.append('line')
					...
					.attr('x2', function(d) {
			    		return margin.left + xScale(d[xVar]) + xScale.rangeBand());
					};
		...
		//allows the user to select a file name
		...
		//gets the minimum of the dataset (lowest dot/line)
		chart.getMin = function() {
			return this;
		}

		//gets the maximum of the dataset (highest dot/line)
		chart.getMax = function() {
			return this;
		}

		//gets the median of the dataset (middle line in box)
		chart.getMedian = function() {
			return this;
		}

		//allows user to choose lines instead of dots for min/max
		chart.useLines = function() {
			return this;
		}
